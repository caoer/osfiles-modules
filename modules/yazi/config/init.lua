...
-- Recolor filenames ONLY for tracked changes. Codes from git.yazi (4dc7f1b):
-- updated=1, deleted=2, added=3, staged=4, unstaged=5, untracked=6, ignored=7,
-- clean=0. 6/7/0 are intentionally absent → untracked/ignored/clean keep
-- filetype color.
...
local git_filename_styles = {
	[5] = ui.Style():fg("blue"),   -- unstaged (worktree modified)
	[4] = ui.Style():fg("cyan"),   -- staged (index modified)
	[3] = ui.Style():fg("green"),  -- added
	[2] = ui.Style():fg("red"),    -- deleted
	[1] = ui.Style():fg("cyan"),   -- updated
}
...
function Entity:style()
	...
	if repo == 99 then
		return s:patch(git_filename_styles[7] or ui.Style())
	end
