...
function M:entry(job)
	...
	elseif shell_value == "nu" then
  	...
  	-- Context-aware .repos exclusion: skip .repos when at a locus root
	local fd_excludes = "--exclude .git --exclude node_modules --exclude target"
	if cwd:match("/locus$") or cwd:match("/locus/$") then
		fd_excludes = fd_excludes .. " --exclude .repos"
	end
